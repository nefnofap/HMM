// The dashboard. Reads the precomputed regime payload (written by the GitHub
// Action that runs the HMM) and renders it. The route is guarded by middleware,
// but we re-check the session here too so the page never renders for a
// non-member even if middleware is misconfigured.
import { redirect } from "next/navigation";
import { auth } from "@/auth";
import regime from "@/data/regime.json";
import type { RegimeData } from "@/lib/regime";
...
export default async function DashboardPage() {
  ...
  const data = regime as RegimeData;
    ...
    <main className="mx-auto min-h-screen w-full max-w-5xl px-5 py-8 sm:px-8">
      ...
      <footer className="mt-12 border-t border-ink-600 pt-5">
        <p className="text-xs leading-relaxed text-ink-500">
          ...
          short-horizon lean, not a guarantee. Keep live trading behind your own
          risk controls.
        </p>
      </footer>
    </main>
