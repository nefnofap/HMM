...
# Streamlit gate
# ----------------------------------------------------------------------
def require_login(render_login=None, render_denied=None) -> Optional[Dict[str, Any]]:
    """
    ...
    Parameters
    ----------
    render_login : callable(authorize_url, invite_url) optional - custom login UI.
    render_denied : callable(user, invite_url) optional - custom denied UI.
    """
    ...
    if code:
        try:
            ...
            # Clear the ?code= from the URL so a refresh doesn't re-exchange.
            st.query_params.clear()
            st.rerun()
        except requests.HTTPError as exc:
            st.error(f"Discord login failed: {exc.response.status_code} "
                     f"{exc.response.text[:200]}")
        except Exception as exc:  # noqa: BLE001
            st.error(f"Discord login failed: {exc}")

    # Not logged in -> show the login screen and stop the rest of the app.
    if render_login:
...
def _default_login(authorize_url: str, invite_url: str) -> None:
    st.markdown(f"### Login required\nThis terminal is gated to members of our "
                f"Discord server. [Login with Discord]({authorize_url})\n\n"
                f"Not a member yet? [Join here]({invite_url}).")


def _default_denied(user: Dict[str, Any], invite_url: str) -> None:
    name = display_name(user)
    st.error(f"Hi {name}, you're not a member of the required Discord server. "
