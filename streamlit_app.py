...
REGIME TERMINAL v2 - a dark "command-center" web dashboard for the HMM market
regime detector.

What's new in v2
----------------
* Category + instrument DROPDOWNS (Crypto / Metals / Forex / Indices /
  Commodities / Stocks) - no typing tickers, plus an optional custom box.
* A big LIVE SIGNAL banner: current regime, confidence, confirmation count,
  and a clear LONG / WAIT / STAND ASIDE verdict.
* A live 8-point confirmation checklist (each indicator shown as PASS/FAIL with
  its current reading), so you can see exactly WHY a signal fired.
* Polished metric cards, equity/drawdown charts, regime list and trade log.
...
from regime_detection import (INSTRUMENT_CATALOG, build_features, load_prices,
                              periods_per_year_for, resolve_ticker,
                              summarise_states)
...
# Friendly labels for the 8 confirmations (key -> display name).
CONFIRMATION_LABELS = {
...
/* Big live signal banner */
.signal {
  border:1px solid var(--border); background:var(--panel); padding:1rem 1.2rem;
...
.signal.long { border-left:5px solid var(--green); }
.signal.long .verdict { color:var(--green); }
.signal.wait { border-left:5px solid var(--amber); }
.signal.wait .verdict { color:var(--amber); }
.signal.stand { border-left:5px solid var(--accent); }
.signal.stand .verdict { color:var(--accent); }
.signal .sub { font-size:.7rem; color:var(--muted); letter-spacing:1px; margin-top:.2rem; }
.signal .right { text-align:right; }
.signal .big { font-size:1.6rem; color:var(--text); }
...
.risk .box { background:var(--accent-dim); color:#fff; font-size:.9rem; width:42px;
...
.chk .name { color:var(--text); }
.chk .val { color:var(--muted); font-size:.66rem; margin-left:.5rem; }
.chk .mark { font-weight:600; }
.chk.pass .mark { color:var(--green); }
.chk.fail .mark { color:var(--accent); }
...
.metric { flex:1; min-width:115px; border:1px solid var(--border);
...
# ----------------------------------------------------------------------
# Control bar: category + instrument dropdowns (+ custom override)
# ----------------------------------------------------------------------
r1c1, r1c2, r1c3 = st.columns([1.3, 2.0, 1.6])
with r1c1:
    category = st.selectbox("CATEGORY", list(INSTRUMENT_CATALOG.keys()), index=0)
with r1c2:
    instrument_names = list(INSTRUMENT_CATALOG[category].keys())
    instrument = st.selectbox("INSTRUMENT", instrument_names, index=0)
with r1c3:
...
# Resolve the chosen instrument to a Yahoo Finance symbol.
if custom.strip():
    symbol = custom.strip()
...
def equity_drawdown_chart(strat_ret, bench_ret):
    ...
    ax1.set_title("EQUITY CURVE (GROWTH OF 1.0)", color="#c9ccd1", fontsize=9,
...
if run_button:
    ...
    # ---- Current (latest bar) status for the live signal banner ----
    current_state = int(states[-1])
    proba = model.predict_state_proba(features)[-1]
    confidence = float(proba[current_state]) * 100.0
    conf_df = confirmation_matrix(df.reindex(features.index),
                                  profile.confirmation_cfg or ConfirmationConfig())
    conf_now = conf_df.iloc[-1]
    n_conf_now = int(conf_now["n_confirmations"])
    last_price = float(df["Close"].reindex(features.index).iloc[-1])

    if current_state == bull_state and n_conf_now >= profile.min_confirmations:
        verdict, vclass, vsub = ("LONG", "long",
                                 "Bull regime + enough confirmations")
    elif current_state == bull_state:
        verdict, vclass, vsub = ("WAIT", "wait",
                                 f"Bull regime, only {n_conf_now}/8 confirmations")
    elif current_state == bear_state:
        verdict, vclass, vsub = ("STAND ASIDE", "stand",
                                 "Bear / crash regime - stay out")
    else:
    ...
    st.markdown(
        f"""
        <div class="signal {vclass}">
          <div>
            <div class="verdict">{verdict}</div>
            <div class="sub">{display_name.upper()} &nbsp;//&nbsp; {vsub}</div>
          </div>
          <div class="right">
            <div class="big">{regime_tag} &middot; {n_conf_now}/8</div>
            <div class="sub">REGIME STATE {current_state} &nbsp;|&nbsp; CONFIDENCE {confidence:.0f}% &nbsp;|&nbsp; LAST {last_price:,.2f}</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    # ---- LEFT: dossier + activity + live confirmation checklist ----
    with left:
        ...
        # Live 8-confirmation checklist with current readings.
        readings = latest_readings(df.reindex(features.index),
                                   profile.confirmation_cfg or ConfirmationConfig())
        rows = ""
        for key, lbl in CONFIRMATION_LABELS.items():
            ok = bool(conf_now[key])
    ...
    # ---- CENTER: regime chart + backtest charts ----
    with center:
        ...
        try:
            ...
            st.markdown(panel(f"BACKTEST <span>// {profile_name}</span>",
    ...
    # ---- RIGHT: regime list + forecast + trade log ----
    with right:
        items = ""
        ...
        for f in forecast["forecast"]:
            ...
            fc += kv(f"t+{f['step']}",
                     f"E[ret] {f['expected_observation'][0]:+.5f} | st {ml}")
        ...
        if isinstance(trades_df, pd.DataFrame) and not trades_df.empty:
            show = trades_df[["entry_time", "exit_time", "leveraged_return_pct",
                              "exit_reason"]].copy()
            ...
            st.dataframe(show.tail(12), use_container_width=True, height=300,
                         hide_index=True)

    st.caption(
        "In-sample backtest (model fit on full window). For honest out-of-sample "
        "results use walkforward.walkforward_backtest in code. The LIVE SIGNAL is "
        "the model's view of the latest bar, not a recommendation. "
        "Research only - not financial advice."
    )
else:
    st.markdown(
        "<div class='panel'><div class='panel-h'>STANDBY</div>"
        "<div class='kv'><span class='k'>Pick a CATEGORY and INSTRUMENT above, "
        "then press <b style='color:#c0392b'>RUN DETECTION</b>.</span></div></div>",
        unsafe_allow_html=True,
    )
